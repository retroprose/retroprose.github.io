class RectEdit {
    ...
    async init() {
        ...
        this.texture = await PIXI.Assets.load('../images/map.png');
        ...
        window.c = {
            dumpmap: () => {
                const pixelArray = this.pixi.renderer.extract.pixels(this.texture);
                const easyarray = [];
    ...
    update(delta) {
        ...
        if (this.select.state == 'drag') {
            ...
            const graphics = this.next();
            graphics.context = this.getContext(width, height);
            graphics.visible = true;
